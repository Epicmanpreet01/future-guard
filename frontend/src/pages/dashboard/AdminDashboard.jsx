...
import {
  ...
  getSuccessRate,
  getTotalStudents,
  exportInstituteTableCSV,
} from "../../utils/dashboardUtils.js";
import {
  useAddMentorMutation,
  useRemoveMentorMutation,
  useUpdateMentorMutation,
} from "../../hooks/mutations/mentorMutation.js";
...
const allInstituteDepartments = [
  ...
  "Bachelor in Science (B.Sc.) Computer Science",
  "Bachelor in Science (B.Sc.) Hons.",
  "Bachelor in Science (B.Sc.) Medical & Non-Medical",
  ...
  "Dentistry",
  "Dermatology",
  "Design (Fashion, Industrial, Graphic, etc.)",
  ...
  "Labour Law",
  "Languages (Hindi, Sanskrit, Foreign Languages etc.)",
  "Law (LLB, LLM)",
  ...
  "Management Studies",
  "Marketing",
  "Master in Arts (M.A.) English & History",
  "Master in Arts (M.A.) Music Instrumental, Music Vocal, Economics, Public Administration and Dance",
  "Master in Commerce (M.Com.) Semester System",
  "Master in Science (M.Sc.) Information Technology",
  "Mathematics",
  ...
  "Postgraduate Diploma in Translation (English to Hindi)",
  "Psychiatry",
  "Psychology",
...
export default function AdminDashboard({ authUser }) {
  ...
  const { mutate: addMentorMutate, isPending: addMentorPending } =
    useAddMentorMutation();
  const { mutate: updateMentorMutate, isPending: updateMentorPending } =
    useUpdateMentorMutation();
  const { mutate: removeMentorMutate, isPending: removeMentorPending } =
    useRemoveMentorMutation();
  ...
  const stats = {
    ...
    avgSuccessRate:
      Math.round(instituteSuccessStats / instituteRiskStats?.high) * 100 || 0,
  ...
  const handleUpdateMentor = () => {
    updateMentorMutate({
      ...
      status: selectedMentor.activeStatus,
    });
  ...
  useEffect(() => {
    const isModalOpen =
      showAddMentorModal || showEditMentorModal || showDepartmentsModal;
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans flex flex-col">
      <DashboardHeader
        ...
        user={{
          name: authUser.name,
          institution: authUser.instituteId.instituteName,
        }}
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            ...
            value={stats.totalHighRiskStudents}
            color="text-red-500"
          />
        </section>
...
const MentorAnalytics = ({ mentors, aggregations }) => {
  ...
  const topDepartments = Object.entries(departmentCounts)
    ...
    .slice(0, 6);
  const maxTopDepartmentCount =
    topDepartments.length > 0 ? topDepartments[0].count : 1;
  const activeInactiveData = {
    active: aggregations?.mentor?.active,
    inactive: aggregations?.mentor?.inactive,
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      ...
      <div className="border-t border-gray-200 pt-6">
        <h4 className="text-sm font-semibold text-gray-700 mb-4">
          Top Departments by Mentor Count
        </h4>
                ...
                <div className="w-1/2 flex items-center">
                  <div className="flex-grow bg-gray-200 rounded-full h-4 mr-2">
                    <div
                      ...
                      style={{
                        width: `${(dept.count / maxTopDepartmentCount) * 100}%`,
                      }}
                    ></div>
                  </div>
                  <span className="font-bold text-teal-500 w-8 text-right">
                    {dept.count}
                  </span>
                </div>
              </div>
  ...
  <div className="bg-white rounded-xl shadow-sm border border-gray-200">
    <div className="px-6 py-4 border-b border-gray-200">
      ...
      <div className="mt-4 relative">
        ...
        <input
          type="text"
          placeholder="Search mentors by name, department, or ID..."
          value={searchTerm}
...
const AdminSidebar = ({ onShowDepartments, aggregations, mentors }) => {
  ...
  const analyticsData = {
    riskDistribution: [
      {
        risk: "High Risk",
        count: aggregations?.risk?.high,
        color: "bg-red-200",
      },
      {
        risk: "Medium Risk",
        count: aggregations?.risk?.medium,
        color: "bg-yellow-300",
      },
      {
        risk: "Low Risk",
        count: aggregations?.risk?.low,
        color: "bg-green-200",
      },
    ],
    departmentPerformance: topDepartments,
  };
    ...
    <aside className="space-y-6">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-3">
          <ActionButton
            icon={BookOpen}
            text="All Departments Info"
            onClick={onShowDepartments}
          />
          <ActionButton
            icon={Download}
            text="Export Reports"
            onClick={() => exportInstituteTableCSV("mentorTable", "mentor")}
          />
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <div key={item.risk}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2.5">
                <div
                  ...
                  style={{
                    width: `${(item.count / totalRiskStudents) * 100}%`,
                  }}
                ></div>
              </div>
            </div>
...
const DepartmentsModal = ({ show, onClose, mentors }) => {
  ...
  const departmentsWithCounts = allInstituteDepartments.map((name) => ({
    name,
    count: mentorCounts[name] || 0,
  }));
  ...
  return (
    <Modal
      show={show}
      onClose={onClose}
      title="All Institute Departments"
      maxWidth="max-w-4xl"
    >
        ...
        {filteredDepartments.map((dept) => (
          ...
          >
            ...
            <div className="w-1/2 flex items-center">
              ...
              <span className="font-bold text-green-500 w-8 text-right">
                {dept.count}
              </span>
            </div>
          </div>
